    ...
    public class Editor_Config : Editor_Common
    {
        ...
        void SetDirty(DirtyFlags flags)
        {
            if (m_IsUsedInstance)
            {
                ...
                if (flags.HasFlag(DirtyFlags.AllBeamGeom)) EditorExtensions.SetAllBeamGeomDirty();
                if (flags.HasFlag(DirtyFlags.AllMeshes)) EditorExtensions.SetAllMeshesDirty();
            }
        }
        ...
        public static void DrawRaymarchingQualitiesPopup(Config instance, SerializedProperty prop, GUIContent content)
        {
...
#if VLB_DEBUG
            {
                ...
                EditorGUILayout.LabelField(string.Format("Serialized Unique ID: {0} | Found Unique ID: {1} / Steps: {2}", prop.intValue, qual.uniqueID, qual.stepCount));
            }
#endif // VLB_DEBUG
        }
        #endregion
