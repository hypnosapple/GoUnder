...
CGINCLUDE
    ...
    uniform sampler2D_float _HydroDepthTex;
...
//        float rawDepthWater = SAMPLE_DEPTH_TEXTURE(_HydroDepthTex, UnityStereoTransformScreenSpaceTex(i.uv_depth));

//        if( rawDepthWater < rawDepth ) rawDepth = rawDepthWater;
        ...
        float3 temp = testRay * abs( _CameraWS.y / testRay.y );
        float waterPlaneDist = length( temp );
//        if( testRay.y > -0.001 ) waterPlaneDist = 0;//1e+25;
